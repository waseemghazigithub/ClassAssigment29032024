let password:string='WaseemGhazi123!'
let username:string='Waseem'

if (uservalid(username)) {console.log("User Name validate")}
...
case 5:
   ...
   else{console.log("Invalid User or Password")}

// Checking strong password
function strongpassword(pwd:string){
    let Pvalid=0;
    if (pwd.length<6)
        {return(1)}
    else if(!/[A-Z]/.test(pwd)){ return(2)}
    else if(!/[a-z]/.test(pwd)){ return(3)}
    else if(!/\d/.test(pwd)){ return(4)}
    else if(!/[$@$!%?&^_-]/.test(pwd)){ return(5)}
    else{return(0)}
   }
// funciton user listed
function uservalid(user:string){
 let Uvalid=false;
 if(user=="Waseem") {Uvalid=true; 
    return Uvalid;
  }
